	...
	<head>
		...
		<script type="text/javascript">
			...
			function openPic(picPath){
				window.open("<%=request.getContextPath()%>"+picPath);  
			}
			...
			<div class="panel-body panel-body-noheader panel-body-noborder" style="width: 100%; height: 100%;">
				<div class="p_20">
					<div class="search_div">
						<form:form action="page" modelAttribute="page" id="queryForm">
							...
							<table class="table_list_3">
								<tr>
									<td class="t_l_3_title">银行名称：</td>
									<td><form:input path="mapBean['bankName']" maxlength="40"/></td>
									<td align="left">
		    		...
		    		<div class="button_div">
						<a href="#" onclick="javascript:EOpenWindow('<c:url value='/payment/cardService/addOrEditCard'/>', {title:'添加',height:400}, refreshPageByFormId)" 
						...
						<form id="pageList">
							<c:forEach items="${page.res }" var="res" varStatus="s">
								<tr>
									...
									<td>${res.bankName}</td>
									<td><a herf="#" onclick="openPic('${res.picPath}')" >${res.picPath} </a></td>
									<td>
										<fmt:formatDate value="${res.updateDate }" pattern="yyyy-MM-dd HH:mm:ss"/>
									</td>
									<td>
										<a href="#" onclick="javascript:EOpenWindow('<c:url value='/payment/cardService/addOrEditCard?id=${res.id }'/>',{title:'${res.bankName}[修改]',height:400}, refreshPageByFormId)"><span>修改</span></a>
									</td>
									<td>
